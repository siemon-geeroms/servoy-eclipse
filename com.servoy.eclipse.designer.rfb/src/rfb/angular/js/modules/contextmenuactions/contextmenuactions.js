angular.module('contextmenuactions', [ 'contextmenu', 'editor' ]).value("SHORTCUT_IDS", {
...
}).factory("$contextmenuactions", function($rootScope, $pluginRegistry, $contextmenu, $editorService, EDITOR_EVENTS, SHORTCUT_IDS) {
    return {
	load : function() {
		...
		promise.then(function(shortcuts) {
			...
			getIconStyle : function() {
			    return {
				'background-image' : "url(images/th_horizontal.gif)"
			    };
			},
			shortcut : shortcuts[SHORTCUT_IDS.SET_TAB_SEQUENCE_ID],
			getItemClass : function() {
			    if (!editorScope.getSelection() || editorScope.getSelection().length < 2)
				return "disabled";
			},
			execute : function() {
			...
			getIconStyle : function() {
			    return {
				'background-image' : "url(images/same_width.gif)"
			    };
			},
			...
			getIconStyle : function() {
			    return {
				'background-image' : "url(images/same_height.gif)"
			    };
			},
			...
			getItemClass : function() {
			    if (!hasSelection(1) || !editorScope.isAbsoluteFormLayout())
				return "disabled";
			},
			execute : function() {
			    setAnchoring(1);
			...
			getIconStyle : function() {
			    return {
				'background-image' : "url(images/bring_forward.png)"
			    }
			},
			...
			getIconStyle : function() {
			    return {
				'background-image' : "url(images/send_backward.png)"
			    }
			},
			...
			getIconStyle : function() {
			    return {
				'background-image' : "url(images/bring_to_front.png)"
			    }
			},
			...
			getIconStyle : function() {
			    return {
				'background-image' : "url(images/send_to_back.png)"
			    }
			},
