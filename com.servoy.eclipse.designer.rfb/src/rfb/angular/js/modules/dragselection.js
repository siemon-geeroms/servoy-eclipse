angular.module('dragselection', [ 'mouseselection' ]).factory('$dragselection', function($rootScope, $pluginRegistry, $editorService, $selectionUtils) {
    return {
	load : function() {
	    $selectionUtils.load();
			...
			if (dragging) {
				...
				if (!editorScope.isAbsoluteFormLayout()) {
					...
					if (!canDrop.dropAllowed) {
					    // full refresh the editor content,
					    // it can be moved to different
					    // places already.
		...
		$('body').keyup(function(event) {
		    // if control is released during drag, the copy is deleted
		    // and selected element must be moved
