angular.module('highlight', [ 'editor' ]).factory('$highlight', function($pluginRegistry, $editorService, $selectionUtils, $timeout) {
	...
	load : function() {
		...
		function drawHighlightDiv() {
			...
			if (node.parentElement.parentElement !== editorScope.glasspane) {
			    if (node.clientWidth == 0 && node.clientHeight == 0 && node.firstChild)
		...
		editorScope.registerDOMEvent("mousemove", "CONTENTFRAME_OVERLAY", onmousemove); // real
												...
												// content
												// iframe
		editorScope.registerDOMEvent("mousedown", "CONTENTFRAME_OVERLAY", disableHighlightDiv); // real
													// selection
													// in
													// editor
													// content
													// iframe
		editorScope.registerDOMEvent("mouseup", "CONTENTFRAME_OVERLAY", enableHighlightDiv); // real
													// selection
													// in
													// editor
													// content
