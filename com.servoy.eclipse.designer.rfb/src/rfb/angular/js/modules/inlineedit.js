angular.module('inlineedit', [ 'editor' ]).factory(
	'$inlineedit',
	function($pluginRegistry, $editorService) {
	    return {
		load : function() {
			...
			function handleDirectEdit(nodeId, model, property, propertyValue, scrollX, scrollY) {
				...
				x : model.location.x + scrollX,
				y : model.location.y + scrollY
			    });
			    ...
			    $("#directEdit").unbind('blur').unbind('keyup').unbind('keydown').html(propertyValue).css({
				display : "block",
			...
			editorScope.registerDOMEvent("dblclick", "CONTENTFRAME_OVERLAY", function(event) {
				...
				var clickPosition = editorScope.convertToContentPoint({
				    x : event.pageX,
				    y : event.pageY
				});
