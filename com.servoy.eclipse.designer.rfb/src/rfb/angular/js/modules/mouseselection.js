angular.module('mouseselection', [ 'editor' ])
	.factory(
		"$selectionUtils",
		function($rootScope, $pluginRegistry, EDITOR_CONSTANTS) {
			...
			for (var i = 0; i < selection.length; i++) {
			    if (selection[i].getAttribute("svy-id") == ghost.getAttribute("svy-id"))
				return true;
			}
			return false;
		    }
		    ...
		    var getUtilsForScope = function(editorScope) {
			return {
				...
				if (type == "layout" || (type == "component" && !editorScope.isAbsoluteFormLayout())) {
				    ...
				    if (!dropTarget) {
					// this is on the form, can this layout
					// container be dropped on the form?
					if (!topContainer) {
					    return {
						dropAllowed : false
					    };
					...
					if (realDropTarget == null) {
						...
						// this is in the 30% corner
						// (bottom or right) of the
						// component
						// the beforeChild should be a
						// sibling of the dropTarget (or
						// empty if it is the last)
						dropTarget = dropTarget.nextElementSibling;
						// if there is no
						// nextElementSibling then force
						// it to append so that it is
				...
				if (lassoStarted || hasClass(event.target, "contentframe-overlay") || hasClass(event.target, "ghost") || hasClass(event.target, "knob")
				...
				if (elements.length == 1)
				    ...
				    if (ghostObject && ghostObject.type == EDITOR_CONSTANTS.GHOST_TYPE_FORM
					    && !(angular.element(elements[elements.length - 2]).is("[svy-non-selectable]"))) {
					return elements[elements.length - 2];
				    }
				    // always return the one on top (visible);
				    // this is due to formIndex implementation
			...
			load : function() {
				...
				var utils = getUtilsForScope(editorScope);
				...
				function select(event, node) {
					...
					/*
					 * Shift-Click does range select: all
					 * elements within the box defined by
					 * the uttermost top/left point of
					 * selected elements to the uttermost
					 * bottom-right of the clicked element
					 */
					...
					// if we right click on selected element
					// while multiple selection,just show
					// context menu and do not modify
					// selection
					return;
				    } else {
					editorScope.setSelection(node);
				    ...
				    // set the focus on the target so that the
				    // editor area scrolls to that element
				    // this is also a workaround for IE bug: if
				    // the user clicks the editor area scroll
				    // bar then nothing else will ever gain
				    // focus
				    // => keyboard arrows will just scroll
				    // instead of moving selected elements
				...
				function onmousedown(event) {
				    ...
				    if (node) {
					if (editorScope.getSelection().indexOf(node) !== -1) {
					    // its in the current selection,
					    // remember this for mouse up.
			...
			getUtilsForScope : getUtilsForScope
		    }
		});
